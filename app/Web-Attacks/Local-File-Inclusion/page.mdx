...
File Inclusion happens when an application builds a file path from user-controlled input. It then passes that path to a function that reads, renders, or executes that file. If the path is not constrained to an intended allow-list, an attacker can break out of the expected directory. The attacker may read sensitive local files, disclose source code, poison readable files, or include attacker-controlled code.
...
Impact depends on two properties. A read-only sink leaks files and source. An executing sink runs included code. Anything the attacker can write into a readable file, such as an upload, session, access log, or remote script, can become code execution.
...
- Noise: medium-high.
- Telemetry: access/error-log payload strings (`../`, encoded traversal, `php://`, `data://`, `expect://`, `phar://`), WAF traversal signatures, anomalous `User-Agent`/cookie values containing `<?php`, outbound HTTP/FTP/SMB callbacks, and new files in upload or web directories.
- Prereq: reachable file-path input and the right sink capability.
- Footprint: poisoned logs/sessions persist, uploaded shells remain, reverse-shell callbacks are observable. Clean up files and report poisoned logs.
