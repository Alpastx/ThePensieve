...
<Callout type="info">
  If nothing executes after exhausting the above, do this:

  - Re-check the **context**. You may be encoding for HTML when the sink is a JS string or attribute.
  - Try **case/whitespace/encoding** evasions against a filter.
  - Confirm there is no **CSP** blocking inline/remote scripts (`Content-Security-Policy` response header).
  - Review client-side JS for a **DOM sink** you can reach directly.
  - Then re-rank inputs and retry the simplest breakout first.
</Callout>
...
<Quiz
  ...
  answer={1}
  explanation="HTML-entity encoding on output is the correct defense for the HTML body context, so a naive payload will not fire here. That does not mean the whole app is safe. Other inputs, other contexts (attribute, JS string, URL), or a client-side DOM sink may still be exploitable. Blindly firing thousands of payloads at an encoded sink wastes time. Reason about context first."
/>
