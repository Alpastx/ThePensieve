...
- [ ] Fallback B `impacket-atexec <USER>:<PASS>@<IP> "<CMD>"` runs over fixed TCP 445 (scheduled task). It survives a blocked dynamic-port range at the cost of scheduled-task events (4698/4702).
...
<Quiz
  ...
  answer={2}
  explanation="psexec and smbexec both install a service via svcctl, which the IPS is blocking. They are dead on arrival. Default wmiexec works but leaves the `cmd.exe /Q /c` process tree and the `ADMIN$\\__<ts>` output file, both signature-matched by mature EDR. `-silentcommand` removes the wrapper and `-no-output` removes the ADMIN$ write. The only remaining artifact is the WMI method call on a completely different transport (135 + dynamic) than the scrutinized SMB path. The key lesson: WMI's value is not absolute stealth. It is a different transport layer that survives when SMB execution is blocked."
/>

## Overview

WMI (Windows Management Instrumentation) is Microsoft's implementation of WBEM and CIM. It is a local and remote management framework exposed as **providers** (DLLs that publish system data) grouped into **namespaces**. Examples: `root\cimv2` for general OS data, `root\SecurityCenter2` for AV state, `root\directory\LDAP` for AD data on DCs. To an attacker it is three things at once: an execution primitive (`Win32_Process.Create`), an enumeration surface (queryable via WQL), and a persistence mechanism (event subscriptions).
...
Every Impacket execution tool achieves the same end goal: a command running on the target. They differ in transport, privilege requirement, detection surface, and output retrieval. Choosing the right one is an OPSEC decision, not a convenience one.
