...
description: "Enumerate and attack SNMP. Cover community string guessing, snmpwalk MIB extraction, credential harvesting from process arguments, and read-write abuse."
openGraph:
  title: "SNMP - Ports 161, 162 (UDP)"
  description: "Enumerate and attack SNMP. Cover community string guessing, snmpwalk MIB extraction, credential harvesting from process arguments, and read-write abuse."
  images:
...
**Recovery (methodology exhausted):** re-confirm UDP/161 actually responds to *something* (a single working community proves it). Widen the community wordlist. Retry with `snmpbulkwalk` if `snmpwalk` stalls on a fat MIB. Re-grep the saved full dump for credentials you skimmed past. Then re-rank: credentials first, known-vuln software second, topology last.
...
<Quiz
  ...
  answer={1}
  explanation="The credential is the finding, not 10.10.20.5. Before chasing a host you cannot yet reach, exhaust the credential against assets you already have network access to. Credential reuse is the single highest-yield technique on internal engagements, and a backup service account is exactly the kind of credential reused on domain-joined hosts. Scanning an out-of-subnet host without a pivot wastes time. Assuming public works there ignores the credential you already hold. A shell via Net-SNMP extend on a host you already read via SNMP adds little. Spray first, pivot second."
/>

## Overview

SNMP (Simple Network Management Protocol) is the standard protocol for monitoring and managing network-connected devices. That includes routers, switches, firewalls, servers, printers, UPSes, IP cameras, and ICS. It uses a manager/agent model. The managed device runs an **agent** that exposes system state as a tree of numeric **OIDs** (Object Identifiers). The manager (an NMS like Zabbix, LibreNMS, Nagios, PRTG, SolarWinds) queries those OIDs over UDP.
