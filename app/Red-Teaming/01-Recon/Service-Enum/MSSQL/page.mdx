...
description: "Enumerate and attack Microsoft SQL Server on port 1433. Cover authentication modes, misconfigurations, data access, xp_cmdshell execution, NTLM hash capture, linked-server lateral movement, and in-engine privilege escalation."
openGraph:
  title: "MSSQL - Port 1433"
  description: "Enumerate and attack Microsoft SQL Server on port 1433. Cover authentication modes, misconfigurations, data access, xp_cmdshell execution, NTLM hash capture, linked-server lateral movement, and in-engine privilege escalation."
  images:
...
- [ ] `nmap --script ms-sql-ntlm-info -p1433 <IP>` extracts hostname, NetBIOS name, DNS domain, forest name, and OS build **without credentials**. MSSQL leaks NTLM challenge data to any anonymous connection. That gives free domain intelligence before you touch auth. On an external engagement this is often the first proof you have landed in a domain-joined environment. The DNS domain is what you need to start Kerberos attacks after a foothold.
...
- [ ] Check for a bi-directional loop (ServerA → ServerB → ServerA with escalated context). That loop can hand you sysadmin on your original box even if you started without it.
...
<Quiz
  ...
  answer={2}
  explanation="Enabling xp_cmdshell requires sysadmin, which you do not have on the current instance. Option A fails immediately. Option B is valid and you should do it eventually, but it may return a local service account hash that will not help if the target is sandboxed. Option D is nonsense: links do not give you an auth oracle. Option C is the critical move. Linked servers frequently authenticate with stored sysadmin credentials. A single query tells you whether you have just inherited full control of PROD_DB without any further escalation. If IS_SRVROLEMEMBER returns 1 on the remote server, you have completed the entire privilege escalation step in one query. Then run commands through the link via EXECUTE (...) AT [PROD_DB]. This is the CPTS-classic MSSQL pivot and it fails silently if you do not check."
/>

## Overview

MSSQL is Microsoft's flagship relational database server, deeply integrated with Windows and Active Directory. It is ubiquitous in Microsoft-heavy enterprises and appears on virtually every CPTS-style internal engagement. That tight Windows integration is also its weakness. SQL logins share the NT auth subsystem. Service accounts are often domain users. The engine exposes primitives (`xp_cmdshell`, `xp_dirtree`, linked servers) that bridge database access into OS and domain compromise.
...
The subnet sweep is the hidden weapon of `nxc mssql`. Once you crack a single SQL login, spraying it across a `/24` routinely reveals other instances sharing the same password. That is common wherever DBAs reuse one "SQL admin" account across dev, staging, and production.
