---
title: RDP - Port 3389
description: "Enumerate and attack Remote Desktop Protocol on port 3389. Cover pre-auth intelligence, NLA and encryption checks, credential spraying, pass-the-hash, pass-the-ticket, file transfer, and session hijacking."
openGraph:
  title: "RDP - Port 3389"
  description: "Enumerate and attack Remote Desktop Protocol on port 3389. Cover pre-auth intelligence, NLA and encryption checks, credential spraying, pass-the-hash, pass-the-ticket, file transfer, and session hijacking."
  images:
...
- [ ] Because RDP gives a real desktop, launch the user's browser and read saved passwords from the GUI password manager directly. That is faster than running sharpchrome on a hardened target.
- [ ] When you hold both RDP *and* WinRM creds for the same target, default to **WinRM via evil-winrm**. It is faster and scriptable. It uses Logon Type 3 (Network) instead of Type 10 (RemoteInteractive), native file transfer, and no GUI cache artifacts. Use RDP only when you specifically need the GUI Credential Manager UI, browser password manager, a thick-client app, or visual reporting evidence.
...
Once you have local admin (and thus SYSTEM via `psexec -s`, token theft, or a service install), other users' RDP sessions on the same host become accessible. You do not need their passwords.
