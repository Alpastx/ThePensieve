...
- [ ] `dig axfr <DOMAIN> @<IP>` attempt the transfer on the primary. A misconfigured `allow-transfer` dumps the entire zone. That dump is the most valuable artifact in DNS recon.
...
- [ ] Check whether TCP 53 is reachable. If only UDP is exposed, zone transfers are blocked at the firewall regardless of server config. A failed AXFR earlier may be a network control rather than a hardened ACL.
...
DNS resolves domain names to IP addresses through a distributed hierarchical system. It is the foundation of every network. It is a fundamental protocol for recon because it is almost always exposed. It also leaks more about a target's internal structure than any other service.
### Server Types
...
Zone transfers replicate an entire DNS zone between servers. They exist so secondary nameservers can sync from a primary. If `allow-transfer` is misconfigured to allow any IP, you receive a complete dump of every record in the zone. That dump includes hostnames, IPs, mail servers, SRV records, and TXT comments. It is the most valuable misconfiguration in DNS.
...
Reverse lookup sweeps reveal hosts that are not in the forward zone but still have PTR records. Those hosts are typically infrastructure devices, monitoring systems, and forgotten servers.
...
If a CNAME record points to an external service (AWS S3, GitHub Pages, Heroku, Azure) and that service is no longer claimed, you can register the dangling resource. You then control the subdomain. Anyone visiting `subdomain.target.com` then lands on your content. That is useful for phishing and for bypass of CSP/CORS restrictions tied to the parent domain.
