...
description: "Enumerate and attack NFS (ports 111 and 2049). Cover export discovery, UID squatting, no_root_squash root escalation, and credential pillaging."
openGraph:
  title: "NFS - Ports 111(UDP/TCP), 2049"
  description: "Enumerate and attack NFS (ports 111 and 2049). Cover export discovery, UID squatting, no_root_squash root escalation, and credential pillaging."
  images:
...
- [ ] **Match the primitive to the exposed path.** An export mapped to `/home` (not `/`) rules out `/etc/cron.d/` and `/root/.ssh/`. You can only reach user home directories. The clean play is dropping your key in `/home/<user>/.ssh/authorized_keys` for every user whose directory exists. A SUID-bash drop is useless there until you already have a foothold to execute it.
...
- [ ] **`no_subtree_check` root escape** (fallback when the export is read-only or maps to a boring subtree): `nxc nfs <IP>` flags this as `Root escape: Yes`. The option (the modern default, enabled for performance) means the server does not verify that a submitted file handle lives inside the exported subtree. A crafted handle can reach `/etc/shadow`, `/root/.ssh/`, or any inode. Use file-handle tooling such as [`libnfs`/`nfs-cat`](https://github.com/sahlberg/libnfs). It is fiddly, so prefer the writable/`no_root_squash` paths first.
