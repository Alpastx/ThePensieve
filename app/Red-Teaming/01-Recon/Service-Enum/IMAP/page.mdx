...
description: "Enumerate and attack IMAP and POP3 mail access protocols. Cover capability fingerprinting, credential attacks, mailbox pillaging, and NTLM hash capture."
openGraph:
  title: "IMAP / POP3 - Ports 143, 993, 110, 995"
  description: "Enumerate and attack IMAP and POP3 mail access protocols. Cover capability fingerprinting, credential attacks, mailbox pillaging, and NTLM hash capture."
  images:
...
<Quiz
  ...
  answer={1}
  explanation="A 'creds backup' email is high value, but the highest-EV move is parallel spraying. Service-account passwords in this kind of dump are routinely reused across SSH/WinRM/SMB/RDP. netexec fires all of them in seconds and often surfaces admin access on hosts the user was not supposed to touch. Hit the named services (MySQL, WordPress) right after, but never skip the spray phase. WordPress RCE is useful, but credential reuse is what gives you footing on *other* hosts."
/>
...
**IMAP** (Internet Message Access Protocol) is a standard email protocol that stores email messages on a mail server. It lets the end user view and manipulate them as though they were stored locally. Unlike POP3, IMAP synchronizes email across multiple devices. Folder hierarchies, search, partial fetches, and flags are all server-side operations.

**POP3** (Post Office Protocol version 3) retrieves emails from a remote server to a local client. It typically downloads messages and deletes them from the server (configurable). POP3 is simpler than IMAP but has fewer features. It is designed primarily for offline email access. It has no folder concept, no server-side search, and only an INBOX view.
