...
description: "Enumerate and attack the rsync daemon on port 873. Cover module discovery, anonymous access, credential attacks, selective looting, writable-module RCE, and credential pillaging."
openGraph:
  title: "Rsync - Port 873"
  description: "Enumerate and attack the rsync daemon on port 873. Cover module discovery, anonymous access, credential attacks, selective looting, writable-module RCE, and credential pillaging."
  images:
...
- [ ] Read every module name/description carefully. Names like `backup`, `home`, `www`, `dev`, `etc`, `secret`, and `keys` are intelligence in themselves, and each implies a different target.
...
- [ ] If `use chroot = no`, plant a symlink to an out-of-module file and read it back. That is a full filesystem read primitive for any file the daemon UID can reach.
...
With `use chroot = yes` (the default), rsyncd `chroot()`s into the module path and cannot serve files outside it. Set to `no`, the daemon runs in its parent's filesystem view. Then *symlinks within the module can point outside it*. Combined with anonymous access this is a full filesystem read primitive (see the symlink commands in Phase 5). This is rare in modern Debian/Ubuntu installs (which default to `yes`). It is common on appliances and NAS firmware where an admin disabled chroot to make a path work.
...
Rsync over SSH is an operationally pleasant Linux file-transfer tool. It compresses, resumes, preserves permissions/timestamps, and inherits SSH's encryption. It needs no extra service on the target beyond standard SSH.
