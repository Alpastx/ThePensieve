...
```bash
# Nmap — service detection (TCP + UDP)
nmap -sV -sC -p53 <IP>
nmap -sU -sV -p53 <IP>
...
# Version via CHAOS class — DNS has no banner
dig CH TXT version.bind @<IP>
...
- [ ] `dig mx <DOMAIN> @<IP>` — mail servers; feed into SMTP enumeration → [SMTP-25,587,465](/01-Recon-and-Enumeration/Service-Enum/SMTP-25,587,465).
...
- [ ] Add every discovered subdomain to `/etc/hosts` and feed them into vhost fuzzing → [FFUF](/Tools/FFUF). Vhost-based sites serve different content per `Host` header, so IP scans alone miss them.
...
- [ ] Feed DC IPs into LDAP/SMB/Kerberos enum → [SMB-137,138,139-445(samba)](/01-Recon-and-Enumeration/Service-Enum/SMB-137,138,139-445(samba)). SRV records reveal this topology with zero credentials.
