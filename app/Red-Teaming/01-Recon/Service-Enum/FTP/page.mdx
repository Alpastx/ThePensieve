---
title: FTP - Port 21
description: "Enumerate and attack FTP services on port 21. Cover anonymous access, credential attacks, known exploits, and TFTP."
openGraph:
  title: "FTP - Port 21"
  description: "Enumerate and attack FTP services on port 21. Cover anonymous access, credential attacks, known exploits, and TFTP."
  images:
...
- [ ] Spray credentials harvested from other services against FTP. Credential reuse is the most common way into FTP. A database password from a config file, an SNMP community string, or an LDAP bind credential may work as an FTP login.
...
ftp <IP> <PORT>
```
The standard `ftp` client is available on virtually every Linux and Windows system. The port argument is optional and defaults to 21. The built-in client is fine for manual enumeration. It lacks scripted mirroring and reliable error handling. Use `lftp` or `wget` for bulk operations.
...
The FTP bounce attack exploits the `PORT` command, which tells the server where to send data. If the server does not validate the target address, an attacker can instruct it to send data to **any host and port**. That turns the FTP server into a proxy. It can port-scan internal networks, bypass firewall rules, and mask the true source of an attack.
