...
description: "Enumerate and attack MySQL and MariaDB on port 3306. Cover empty-password access, default credentials, file read/write to RCE, UDF execution, and hash harvesting."
openGraph:
  title: "MySQL - Port 3306"
  description: "Enumerate and attack MySQL and MariaDB on port 3306. Cover empty-password access, default credentials, file read/write to RCE, UDF execution, and hash harvesting."
  images:
...
- [ ] Grep web app configs first. `wp-config.php`, `config.php`, and `.env` are the richest source of MySQL credentials on any engagement. DBAs reuse the DB password on SSH/SMB constantly.
...
- [ ] If `INTO OUTFILE` fails, distinguish the cause. An `Access denied` SQL error means no `FILE` privilege (go to Phase 7). A filesystem error means the target dir is not writable by `mysql`. Read the Apache config (`LOAD_FILE('/etc/apache2/sites-enabled/000-default.conf')`) to find a writable `DocumentRoot`, or stage in `/tmp/` for use with an existing LFI.
...
<Quiz
  ...
  answer={2}
  explanation="UDF (A) works but is architecture-dependent, version-sensitive, and blocked on modern MySQL. It is high effort with a high failure rate. Option B is speculative: the MySQL root password may not match OS root, and SSH root password auth is often disabled. Option D (general_log to PHP) is a real but fragile technique requiring precise permissions and surviving SQL escaping. Option C is the textbook path: INTO OUTFILE with empty secure_file_priv is the most reliable MySQL-to-RCE primitive there is one query lands a web shell, one curl confirms it. Save exotic techniques for when the simple path is blocked."
/>
...
MySQL supports several authentication methods. Username/password is the common case. OS-identity plugins (`unix_socket`/`auth_socket`) exist too, and even Windows authentication via a plugin. Administrators pick a mode for compatibility, security, or usability reasons. Each choice introduces its own misconfigurations. Anonymous access, passwordless accounts, or accounts allowed from any host (`user@'%'`) all grant entry without a valid credential.
...
`skip-grant-tables` completely disables authentication. Any client connecting is treated as root with all privileges. It is normally a password-recovery boot flag. If you find it in a running config on a live server, the database is wide open to anyone who can reach port 3306.
</Callout>
