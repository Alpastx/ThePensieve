...
const Folder: FC<FolderProps> = ({ item: _item, anchors, onFocus, level }) => {
  ...
  const sidebarConfig = useThemeConfig().sidebar ?? {}
  const { defaultMenuCollapseLevel = 2, autoCollapse } = sidebarConfig
...
export const MobileNav: FC = () => {
  ...
  const hasI18n = (themeConfig.i18n?.length ?? 0) > 0
...
export const Sidebar: FC = () => {
  ...
  const themeConfig = useThemeConfig()
  const [isExpanded, setIsExpanded] = useState(themeConfig.sidebar?.defaultOpen ?? true)
  ...
  const hasI18n = (themeConfig.i18n?.length ?? 0) > 0
  const hasMenu =
    themeConfig.darkMode || hasI18n || (themeConfig.sidebar?.toggleButton ?? true)
      ...
      >
        ...
        {hasMenu && (
          ...
          >
            ...
            />
            {(themeConfig.sidebar?.toggleButton ?? true) && (
              <Button
